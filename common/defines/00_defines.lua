...
NDiplomacy = {
	...
	BASE_SEND_ATTACHE_MAINTAIN_COST = 0.1,									-- Political power cost each update when sending the attache to another country
	...
	WARGOAL_WORLD_TENSION_REDUCTION = -0.4,									-- Reduction of pp cost for wargoal at 100% world tension, scales linearly # was -0.5
	...
	TENSION_CIVIL_WAR_IMPACT = 0.14,										-- civil war multiplier on tension.
	...
	TENSION_VOLUNTEER_FORCE_DIVISION = 0.04,								-- Amount of tension generated for each sent division
	...
	TENSION_CAPITULATE = 0.6,												-- Scale of the amount of tension created by a countries capitulation.
...
NCountry = {
	...
	SUPPLY_BONUS_FROM_INPUT = 0.2,								-- % of supply bonus from input area.
	...
	RESISTANCE_STRENGTH_FROM_NEIGHBORS = 0.7, 					-- Multiplies how much resistance can spread from one state to its neighbors, a state will spread whatever is highest of its victorypoints resistance increase or half of any of its neighbors spread, multiplied by this
...
NProduction = {
	...
	MAX_CIV_FACTORIES_PER_LINE = 16,							-- Max number of civ-factories that can be assigned a single production line.
	...
	BASE_CONSUMER_GOODS_NEED_FACTOR = 55,  						-- Base factor for the consumer goods need of civilian factories in %.
...
NMilitary = {
	...
	CORPS_COMMANDER_DIVISIONS_CAP = 25,								-- how many divisions a corps commander is limited to. 0 = inf, < 0 = blocked
	CORPS_COMMANDER_ARMIES_CAP = -1,								-- how many armies a corps commander is limited to. 0 = inf, < 0 = blocked
	FIELD_MARSHAL_DIVISIONS_CAP = 5,								-- how many divisions a field marshall is limited to. 0 = inf, < 0 = blocked
	...
	MAX_DIVISION_SUPPORT_HEIGHT = 10,								-- Max height of support in division designer.
	...
	LAND_COMBAT_STR_DAMAGE_MODIFIER = 0.0525,        				-- global damage modifier... but some equipment is returned at end of battles see : EQUIPMENT_COMBAT_LOSS_FACTOR
	...
	LAND_COMBAT_COLLATERAL_FACTOR = 0.0016,		   					-- Factor to scale collateral damage to infra and forts with.
	LAND_COMBAT_FORT_DAMAGE_CHANCE = 1,								-- chance to get a hit to damage on forts. (out of 100)
	ATTRITION_DAMAGE_ORG = 0.15,					   				-- damage from attrition to Organisation
	...
	COMBAT_MOVEMENT_SPEED = 0.2,	               					-- speed reduction base modifier in combat
	...
	BASE_LEADER_TRAIT_GAIN_XP = 0.25,				  				-- Base xp gain for traits per hour.
	...
	TRAINING_ATTRITION = 0.08,		  			   					-- amount of extra attrition from being in training
	...
	LOW_SUPPLY = 0.80,							   					-- DNM, just in case 0.75 makes AI too attrition-happy -- When the supply status of an unit becomes low.
	...
	LAND_COMBAT_ATTACKER_WINNING = 0.7,		  						-- Above this means attacker is winning
	LAND_COMBAT_DEFENDER_WINNING = 0.35,		  					-- Below this means defender is winning
	PLAN_CONSIDERED_GOOD = 0.65,									-- Plan evaluations above this value are considered more or less safe
	PLAN_CONSIDERED_BAD = 0.35,										-- Plan evaluations below this value are considered unsafe
	...
	PLAN_SPREAD_ATTACK_WEIGHT = 13.0,								-- DNM, original 13 (AI overhaul mod) -- The higher the value, the less it should crowd provinces with multiple attacks.
	...
	PLAN_PROVINCE_HIGH_VP_IMPORTANCE_AREA = 12.0,   				-- DNM, original 50 (AI overhaul mod) -- Used when calculating the calue of defense area in the battle plan system
	...
	PLAN_EXECUTE_CAREFUL_LIMIT = 10,								-- When looking for an attack target, this score limit is required in the battle plan to consider province for attack
	PLAN_EXECUTE_BALANCED_LIMIT = -1,								-- When looking for an attack target, this score limit is required in the battle plan to consider province for attack
	...
	PARADROP_HOURS = 36,											-- time paratroopers suffer penalties in combat
...
NAir = {
	...
	COMBAT_DAMAGE_STATS_MULTILIER = 0.31,
	...
	SUPPLY_NEED_FACTOR = 0.36, 								-- multiplies supply usage
...
NAI = {
	...
	DIPLOMACY_SEND_MAX_FACTION = 0.1,						-- Country should not send away more units than this as expeditionaries
	...
	MIN_ANTAGONIZE_FOR_WARGOAL_JUSTIFICATION = 2000,		-- AI countries will not fabricate claims against countries with an antagonization value lower than this.
	RESEARCH_DAYS_BETWEEN_WEIGHT_UPDATE = 1, 				-- Refreshes need scores based on country situation.
	RESEARCH_LAND_DOCTRINE_NEED_GAIN_FACTOR = 3.50, 		-- Multiplies value based on relative military industry size / country size.
	...
	VARIANT_UPGRADE_MIN_XP = 799,							-- Need to have it below 500 since air & navy dont go to 800 -- Minimum XP before attempting to create a new variant.
	UPGRADE_XP_RUSH_UPDATE = 25,							-- If XP is above this on the daily tick the AI will attempt to spend it
	...
	VP_GARRISON_VALUE_FACTOR = 0,							-- Extent to which VP garrisons are prioritized, based on VP value and compared to other priority values.
	...
	LENDLEASE_FRACTION_OF_PRODUCTION = 0.75,				-- Base fraction AI would send as lendlease
	...
	FRONT_REASSIGN_DISTANCE = 100.0,						-- DNM, original 100.0 (ai overhaul mod) -- If a unit is this far away from a front it is not considered to be assigned to it unless the new front is much more important
	...
	DIVISION_DESIGN_COMBAT_WIDTH_WEIGHT = 0,				-- This score is reduced the higher width is when comparing pure changes with no target
	...
	NAVAL_MISSION_MIN_FLEET_SIZE = 30,						-- AI will not send too small fleets on missions. Ignored if total number of ships country has is below	this.
	NAVY_PREFERED_MAX_SIZE = 80,							-- DNM, original 20 - BICE thought 50 was good as well.  AI will generally attempt to merge fleets into this size, but as a soft limit.
	...
	MAX_SUPPLY_DIVISOR = 0.25,								-- DNM, original 2 (AI mod) To make sure the AI does not overdeploy divisions. Higher number means more supply per unit.
	...
	STATE_CONTROL_FOR_AREA_DEFENSE = 0.5,					-- To avoid AI sending area defense to area with very little foothold
	...
	MIN_UNITS_FACTOR_FRONT_ORDER = 2.5,						-- Factor for min number of units to assign to area front orders
	...
	REQUEST_LEND_LEASE_STOCKPILE_RATIO_LAND = 0.8,			-- Ratio to send stockpile from equipment
	REQUEST_LEND_LEASE_PRODUCTION_DAYS_LAND = 50,			-- Count of daily production amount of equipment to send
	REQUEST_LEND_LEASE_STOCKPILE_RATIO_AIR = 0.95,			-- Ratio to send stockpile from equipment
	REQUEST_LEND_LEASE_PRODUCTION_DAYS_AIR = 25,			-- Count of daily production amount of equipment to send
	REQUEST_LEND_LEASE_STOCKPILE_RATIO_NAVAL = 0.5,			-- Ratio to send stockpile from equipment
	REQUEST_LEND_LEASE_PRODUCTION_DAYS_NAVAL = 30,			-- Count of daily production amount of equipment to send
	...
	PLAN_ACTIVATION_SUPERIORITY_AGGRO = 0.9,						-- How aggressive a country is in activating a plan based on how superiour their force is.
	WAIT_YEARS_BEFORE_FREER_BUILDING = 1,							-- After how many years it switches from CIV only to MIC +NIC Only
